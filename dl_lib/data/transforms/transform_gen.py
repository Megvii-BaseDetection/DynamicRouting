...
from .extend_transform import (BlendTransform, BoxJitterTransform,
                               CropPadTransform, CropTransform,
                               DistortTransform, HFlipTransform, NoOpTransform,
...
class ResizeLongestEdge(TransformGen):
    ...
    def get_transform(self, img):
        h, w = img.shape[:2]
        if self.is_range:
            size = np.random.randint(self.long_edge_length[0],
                                     self.long_edge_length[1] + 1)
        else:
...
class RandomContrast(TransformGen):
    ...
    def get_transform(self, img):
        ...
        return BlendTransform(src_image=img.mean(),
                              src_weight=1 - w,
                              dst_weight=w)
