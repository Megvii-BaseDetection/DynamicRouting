...
class Boxes:
    ...
    def cat(boxes_list: List["Boxes"]) -> "Boxes":
        ...
        cat_boxes = type(boxes_list[0])(cat([b.tensor for b in boxes_list],
                                            dim=0))
        return cat_boxes
...
def matched_boxlist_iou(boxes1: Boxes, boxes2: Boxes) -> torch.Tensor:
    ...
    Returns:
        (tensor) iou, sized [N].
    """
    assert len(boxes1) == len(boxes2), ("boxlists should have the same"
                                        "number of entries, got {}, {}".format(
