...
def convert_basic_c2_names(original_keys):
    ...
    layer_keys = [
        re.sub("bn\\.rm", "norm.running_mean", k) for k in layer_keys
    ]
    layer_keys = [
        re.sub("bn\\.running.mean$", "norm.running_mean", k)
        for k in layer_keys
    ]
    layer_keys = [
...
def convert_c2_detectron_names(weights):
    ...
    layer_keys = [
        k.replace("rpn.bbox.pred.fpn2",
                  "proposal_generator.rpn_head.anchor_deltas")
        for k in layer_keys
    ]
    layer_keys = [
        k.replace("rpn.cls.logits.fpn2",
    ...
    for orig, renamed in zip(original_keys, layer_keys):
        ...
        if renamed.startswith("bbox_pred.") or renamed.startswith(
            ...
            logger.info(
                ...
                "{} to {}.".format(renamed, tuple(weights[orig].shape),
...
def align_and_update_state_dicts(model_state_dict,
    ...
    if len(unmatched_ckpt_keys):
        logger.info(
            get_unexpected_parameters_message(original_keys[x]
                                              for x in unmatched_ckpt_keys))
